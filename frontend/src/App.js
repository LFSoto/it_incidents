...
function Chatbot() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setResponses([
        ...responses,
        { query: input, reply: response.data.solution },
      ]);
      setInput("");
    } catch (error) {
  ...
  const handleGPTSubmit  = async (event) => {
    ...
    try {
      ...
      setResponses([
        ...responses,
        { query: input, reply: response.data.content },
      ]);
      setInput("");
    } catch (error) {
      console.error("Error during API call", error);
  ...
  return (
    <div style={styles.container}>
        ...
        {responses.map((exchange, index) => (
          <p key={index}>
            ...
            <div style={{ textAlign: "end" }}>
              ...
              <p>
                <TypeAnimation sequence={[exchange.reply, 1000]} speed={150} />
                <br />
              </p>
            </div>
          </p>
      ...
      <form style={styles.form} onSubmit={(e) => e.preventDefault()}>
        ...
        <button onClick={handleGPTSubmit} style={styles.button}>
          GPT
        </button>
      </form>
    </div>
